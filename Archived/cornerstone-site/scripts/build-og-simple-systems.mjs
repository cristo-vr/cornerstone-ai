...
import {
  mkdirSync, writeFileSync, readFileSync, readdirSync, existsSync, rmSync, unlinkSync,
} from "node:fs";
...
/**
 * The filename carries a hash of the bytes, and the page imports it from the
 * generated module below rather than hardcoding it.
 *
 * WHY. LinkedIn does not just cache the OG tags for a URL (about a week, and
 * Post Inspector is the only way to clear it) — it also REHOSTS the image on
 * media.licdn.com keyed to the image URL. Ship a new card at the same path and
 * even a forced re-scrape can hand back the stored copy of the old bytes. A
 * stable filename is therefore a card you cannot correct once anyone has
 * shared it. A content hash makes every edit a new URL that nothing has seen.
 */
const OUT_DIR = "public/images";
const OUT_STEM = "og-simple-systems";
const OUT_MODULE = "app/simple-systems/og-image.ts";
...
const jpeg = await sharp(pngPath).jpeg({ quality: 92, chromaSubsampling: "4:4:4" }).toBuffer();
const hash = createHash("sha256").update(jpeg).digest("hex").slice(0, 10);
const file = `${OUT_STEM}.${hash}.jpg`;

// Exactly one card is ever on disk. Leaving the old hashes behind would ship
// dead bytes on every deploy and make it ambiguous which one is current.
for (const f of readdirSync(OUT_DIR)) {
  if (f.startsWith(`${OUT_STEM}.`) && f.endsWith(".jpg") && f !== file) {
    unlinkSync(join(OUT_DIR, f));
    console.log(`  removed stale ${f}`);
  }
}

writeFileSync(join(OUT_DIR, file), jpeg);
writeFileSync(
  OUT_MODULE,
...
` +
    `export const OG_IMAGE = "/images/${file}";
`,
  "utf8",
);
console.log(`  ${OUT_DIR}/${file}: ${W}x${H}, ${(jpeg.length / 1024).toFixed(1)} kB`);
console.log(`  ${OUT_MODULE} -> /images/${file}`);
