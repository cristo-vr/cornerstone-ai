...
import { Lock, ScrollText, Database, LayoutDashboard, Check, Minus } from "lucide-react";
...
/* Roles down the side, data across the top. The point isn't the specific grid,
   it's that the grid exists at all: access is a property of the data. */
  ...
  return (
    <div className="w-full rounded-xl border border-line bg-surface/50 p-6 md:p-7 overflow-x-auto">
      ...
      <p className="mt-6 font-semibold text-[10px] uppercase tracking-[0.14em] text-ink-2 leading-relaxed">
        An example. Yours is set to your roles in week one, and you can change it any day.
      </p>
    </div>
  );
};
  ...
  <section className="py-28 md:py-36 border-t border-line">
    <div className="max-w-6xl mx-auto px-6 grid lg:grid-cols-12 gap-12 lg:gap-16 items-start">
      {/* min-w-0: a grid item defaults to min-width:auto, so the matrix table's
          min-width would otherwise push the whole column past the viewport
