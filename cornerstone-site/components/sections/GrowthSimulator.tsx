...
const GrowthSimulator: React.FC = () => {
    const [advisors, setAdvisors] = useState(12); // Start with 12 advisors
    const [showFab, setShowFab] = useState(false);
    const sectionRef = React.useRef<HTMLElement>(null);
    const [isAnimating, setIsAnimating] = useState(false);

    const maxAdvisors = 50;
    ...
    const addAdvisor = () => {
        if (advisors < maxAdvisors) {
            setAdvisors((prev) => prev + 1);
            setIsAnimating(true);
    ...
    const calculateMetrics = (isCornerstone: boolean) => {
        ...
        const adminStaff = isCornerstone
            ...
            : Math.ceil(advisors / 5) + 1; // Manual: 1 admin per 5 advisors + base

        // Data Integrity Calculation (0-100%)
        const dataIntegrity = isCornerstone
            ? Math.min(92 + (advisors * 0.1), 99) // High and stable
            : Math.max(90 - (advisors * 1.5), 20); // Degrades rapidly with scale

        // Owner Hours on Ops Calculation (Hours/week)
        const ownerHoursOnOps = isCornerstone
            ? Math.min(5 + (advisors * 0.2), 10) // Low and flat
            : Math.min(5 + (advisors * 1.2), 60); // Scales linearly to burnout
                    ...
                    <div className="lg:col-span-5 lg:sticky lg:top-24 static">
                        <div className="mb-6 lg:mb-12">
                            ...
                            <p className="text-muted text-lg leading-relaxed">
                                Most FSPs break because every new advisor adds admin load that scales linearly. <br />
                                ...
                                {!showFab ? (
                                    <>
                                        ...
                                        >
                                            ...
                                            {advisors >= maxAdvisors ? 'Max Advisors' : 'Onboard New Advisor'} <span className="text-xs lg:text-sm opacity-70 ml-1 bg-black/20 px-2 py-0.5 rounded-full normal-case tracking-normal whitespace-nowrap">(Try Me)</span>
                                        </button>
                    ...
                    <div className="lg:col-span-7 flex flex-col gap-6">
                        ...
                        <div className="bg-neutral-900/50 border border-white/5 p-8 rounded-2xl relative overflow-hidden transition-all duration-300">
                            ...
                            <div className="flex justify-between items-start mb-6">
                                <div>
                                    <h3 className="text-xl font-bold text-muted mb-1">Spreadsheet FSP</h3>
                                    <div className="flex items-baseline gap-2">
                                        <span className="text-3xl font-black text-white">{advisors}</span>
                                        <span className="text-neutral-500 text-sm uppercase tracking-wider">Advisors</span>
                                    </div>
                                </div>
                                ...
                                <div>
                                    <div className="flex justify-between items-center mb-2">
                                        <div className="flex items-center gap-2 text-neutral-400">
                                            ...
                                            <span className="text-xs font-bold uppercase tracking-wide">Data Integrity</span>
                                        </div>
                                        <span className="text-red-400 font-bold text-sm">{Math.round(spreadsheetMetrics.dataIntegrity)}%</span>
                                    </div>
                                    <div className="h-2 w-full bg-neutral-800 rounded-full overflow-hidden mb-1">
                                        <motion.div
                                            className="h-full bg-red-500"
                                            initial={{ width: '90%' }}
                                            animate={{ width: `${spreadsheetMetrics.dataIntegrity}%` }}
                                            transition={{ type: 'spring', stiffness: 50 }}
                                        />
                                    </div>
                                    <p className="text-right text-[10px] text-red-500 uppercase tracking-wider">Degrading</p>
                                </div>
                        ...
                        <div className="bg-gradient-to-br from-neutral-800/80 to-neutral-900/80 rounded-2xl p-8 border border-primary/20 shadow-[0_0_30px_rgba(0,0,0,0.3)] relative overflow-hidden">
                            ...
                            <div className="flex justify-between items-start mb-6">
                                <div>
                                    <h3 className="text-xl font-bold text-white mb-1">Cornerstone FSP</h3>
                                    <div className="flex items-baseline gap-2">
                                        <span className="text-3xl font-black text-primary">{advisors}</span>
                                        <span className="text-neutral-400 text-sm uppercase tracking-wider">Advisors</span>
                                    </div>
                                </div>
                            ...
                            <div className="space-y-8">
                                ...
                                <div>
                                    <div className="flex justify-between items-center mb-2">
                                        ...
                                        <span className="text-primary font-bold text-sm">{Math.round(cornerstoneMetrics.dataIntegrity)}%</span>
                                    </div>
                                    <div className="h-2 w-full bg-neutral-800 rounded-full overflow-hidden mb-1">
                                        <motion.div
                                            className="h-full bg-primary"
                                            initial={{ width: '95%' }}
                                            animate={{ width: `${cornerstoneMetrics.dataIntegrity}%` }}
                                            transition={{ type: 'spring', stiffness: 50 }}
                                        />
                                    </div>
                                    <p className="text-right text-[10px] text-primary uppercase tracking-wider">Locked In</p>
                                </div>
